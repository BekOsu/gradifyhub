...
import { ArrowLeft, BookOpen, MessageSquare, PenLine, Clock } from "lucide-react";
import Link from "next/link";
import { ContinueButton } from "./continue-button";

const FOUNDATION_LESSON_SLUGS = [
  "ss-pr-descriptions-merged-in-24-hours",
  "ss-discovery-call-without-losing-them",
  "ss-loom-written-standups-shipping-without-meetings",
];
...
const LESSON_ICONS: Record<string, React.ElementType> = {
  "ss-pr-descriptions-merged-in-24-hours": PenLine,
  "ss-discovery-call-without-losing-them": MessageSquare,
  "ss-loom-written-standups-shipping-without-meetings": BookOpen,
};

export default async function FoundationPathSSPage() {
  ...
  const ssFoundationCheckScore = aiCalib?.ssFoundationCheckScore as number | undefined;
  const hasCompletedFoundationPath = aiCalib?.ssFoundationPathCompletedAt !== undefined;

  if (hasCompletedFoundationPath) {
    redirect("/roadmap?track=soft-skills");
  }

  if (!ssFoundationCheckScore || ssFoundationCheckScore >= 3) {
    redirect("/roadmap?track=soft-skills");
  }
  ...
  if (lessons.length !== FOUNDATION_LESSON_SLUGS.length) {
    ...
    console.error("[foundation-path-ss] Missing lessons:", missingSlugs);
    return (
    ...
    <div className="mx-auto max-w-2xl space-y-8 pb-16 px-4 sm:px-6 lg:px-8">
      ...
      <div className="space-y-3">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-purple-500/10 px-3 py-1 text-xs font-semibold text-purple-600">
          ...
          Soft Skills · Foundation Path
        </span>
        <h1 className="text-3xl font-bold tracking-tight">Build your foundation</h1>
        <p className="text-base text-muted-foreground max-w-lg">
          Complete these three lessons to unlock the full Soft Skills roadmap.
        </p>
      </div>
      ...
      <div className="space-y-3">
        {foundationLessons.map((l) => {
          ...
          return (
            <Link
              key={l.id}
              href={`/learn/${l.slug}?from=foundation-path-ss`}
              className={`block rounded-lg border p-5 space-y-3 transition-all ${
                isCompleted
                  ? "border-purple-300/40 bg-purple-500/5 hover:bg-purple-500/8"
                  : "border-border bg-card hover:border-border/80 hover:shadow-sm hover:bg-muted/30"
              }`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 flex-1">
                  <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${isCompleted ? "bg-purple-500/20" : "bg-muted"}`}>
                    {IconComponent && <IconComponent className={`h-4 w-4 ${isCompleted ? "text-purple-600" : "text-muted-foreground"}`} />}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      ...
                      {!isCompleted && <span className="text-xs font-medium text-purple-600">Start lesson →</span>}
                    </div>
                  </div>
                </div>
