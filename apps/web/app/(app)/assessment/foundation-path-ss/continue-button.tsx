...
export function ContinueButton({ isEnabled }: { isEnabled: boolean }) {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/actions/foundation-path-complete-ss", {
        method: "POST",
      ...
      router.push(result.redirectUrl ?? "/roadmap?track=soft-skills");
    } catch (err) {
    ...
    <div className="space-y-3">
      ...
      >
        {loading
          ? "Saving progress..."
          : isEnabled
          ? "Continue to Soft Skills roadmap →"
