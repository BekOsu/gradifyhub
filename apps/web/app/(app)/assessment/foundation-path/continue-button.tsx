...
export function ContinueButton({ isEnabled }: { isEnabled: boolean }) {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/actions/start-assessment-post-foundation", {
        method: "POST",
  ...
  return (
    <div className="space-y-3">
      ...
      >
        {loading ? "Starting assessment..." : isEnabled ? "Continue to assessment →" : `Complete all lessons first`}
      </button>
    </div>
  );
}
