...
import { saveFoundationCheckResultsEng } from "~/lib/foundation/storage";

const EXPECTED_QUESTIONS = ["reading", "listening", "speaking", "writing", "cultural"];
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      await saveFoundationCheckResultsEng(session.user.id, answers as Record<string, string>, score);
    } catch (err) {
      console.error("[foundation-check-eng] Failed to save results:", err);
    }

    return NextResponse.json({ success: true, score, needsFoundationPath });
  } catch (error) {
    console.error("[foundation-check-eng] Error:", error);
